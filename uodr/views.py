...
#import rest framework response and api_view
from rest_framework.response import Response
from rest_framework.decorators import api_view

#import models
from .models import Orders
#import serialized models
...
def home(request):
    responseStatus = OrderStatus(request).data
    response_data = getOrder(request).data  # Get data from getOrder view
    return render(request, "home.html", {'response_data': response_data, 'responseStatus': responseStatus})
...
def getOrder(request):
    if request.method =="GET":
        ...
        pending_count = sum(item['count'] for item in data if item['status'] == 'Pending')
        order_confirmed_count = sum(item['count'] for item in data if item['status'] == 'Order Confirmed')
        out_for_delivery_count = sum(item['count'] for item in data if item['status'] == 'Out for Delivery')
        delivered_count = sum(item['count'] for item in data if item['status'] == 'Delivered')

        response_data = [
            {'value': pending_count, 'name': 'Pending'},
            {'value': order_confirmed_count, 'name': 'Order Confirmed'},
            {'value': out_for_delivery_count, 'name': 'Out for Delivery'},
            {'value': delivered_count, 'name': 'Delivered'},
        ]
        #serializer = OrdersSerializer(response_data, many=True)
